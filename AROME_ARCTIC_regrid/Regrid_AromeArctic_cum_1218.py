# Script to regrid AA onto 1km grid with 12:00 UTC init appended to 18
...
def get_arome_path(lead_time, identifier, path_data_task, dd, yyyymmdd):
    return(glob.glob(f"{path_data_task}{dd:02d}/arome_arctic_{identifier}_2_5km_{yyyymmdd}T{lead_time}Z.nc")[0])
...
def main():
    ...
    path_output = '/lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML/AROME_ARCTIC_regrid/Data/appended/'
    ...
    for dd in range(1, nb_days_task + 1):
        ...
        # Full no longer exists
        # path_day = lambda x: glob.glob(f"{path_data_task}{dd:02d}/arome_arctic_full_2_5km_{yyyymmdd}T{x}Z.nc")
        # path_day = get_arome_path(12, 'full', path_data_task, dd, yyyymmdd)
        # path_day_det = lambda x: glob.glob(f"{path_data_task}{dd:02d}/arome_arctic_det_2_5km_{yyyymmdd}T{x}Z.nc")
        # path_day_det = get_arome_path(12, 'det', path_data_task, dd, yyyymmdd)
        # path_day_extracted = lambda x: glob.glob(f"{path_data_task}{dd:02}/arome_arctic_extracted_2_5km_{yyyymmdd}T{x}Z.nc")
        # path_day_sfx = lambda x: glob.glob(f"{path_data_task}{dd:02}/arome_arctic_sfx_2_5km_{yyyymmdd}T{x}Z.nc")

        try:
            dataset_12 = get_arome_path(12, 'det', path_data_task, dd, yyyymmdd) # [0]
            dataset_18 = get_arome_path(18, 'det', path_data_task, dd, yyyymmdd) # [0]
            
        except IndexError:
            try:
                # print(f"{path_data_task}{dd:02}/arome_arctic_extracted_2_5km_{yyyymmdd}T18.nc")
                # print(path_day_extracted)
                dataset_12 = get_arome_path(12, 'extracted', path_data_task, dd, yyyymmdd)
                dataset_18 = get_arome_path(18, 'extracted', path_data_task, dd, yyyymmdd)
        ...
        with Dataset(dataset_12, 'r') as nc:
            t2m_arome_12 = nc.variables['air_temperature_2m'][:6, ...]
            uwind_arome_12 = nc.variables['x_wind_10m'][:6, ...]
            vwind_arome_12 = nc.variables['y_wind_10m'][:6, ...]


        with Dataset(dataset_18, 'r') as nc:
        ...
        for idx, i in enumerate(lead_times):
            t2m_cum_mean[idx] = np.ma.concatenate((t2m_arome_12, t2m_arome[0:i,...])).mean(axis = 0)
            uwind_cum_mean[idx] = np.ma.concatenate((uwind_arome_12, uwind_arome[0:i,...])).mean(axis = 0)
            vwind_cum_mean[idx] = np.ma.concatenate((vwind_arome_12, vwind_arome[0:i,...])).mean(axis = 0) 
        ...
        # No more rotation, see Regrid_AROMEArctic_cum for specifications
        ...
        # Output netcdf file
        ################################################
        output_filename = f"AROME_1kmgrid_{yyyymmdd}T12Z.nc"
