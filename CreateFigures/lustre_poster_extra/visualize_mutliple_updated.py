...
from matplotlib import pyplot as plt, transforms as mtransforms, colors as mcolors, gridspec as gridspec
...
def main():
    ...
    land_cmap = WMOcolors.cm.cartopy_land()
    ...
    ice_ticks = ['ice-free\nopen water', '10', '40', '70', '90', 'fast ice']
    ...
    # ax = {}
    # ax['a'] = fig.add_subplot(gs[1:3, 0], projection = map_proj)
    # ax['b'] = fig.add_subplot(gs[:2, 1], projection = map_proj)
    # ax['c'] = fig.add_subplot(gs[2:, 1], projection = map_proj)
    ...
    ax['c'].set_title(f"Deep learning {vdate.strftime('%d')}th {vdate.strftime('%B')} {vdate.strftime('%Y')}")
    ...
    cbar.set_label(label = 'Sea ice concentration thresholds [%]', size = 16)
    cbar.set_ticks(ice_levels[:-1], labels = ice_ticks, size = 15)
    ...
    # plt.tight_layout()
    print('saving fig')
    # fig.savefig(f"{path_figure}new_predictions_updated.png", dpi = 300)
    fig.savefig(f"{path_figure}new_predictions_updated.pdf", dpi = 300)
