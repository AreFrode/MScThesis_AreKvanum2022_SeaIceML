...
def main():
    ...
    icefile0 = 'icefile0'
    if not os.path.isfile(f"{path_figure}{icefile0}.npy"):
        print('creating edge0')
        ice_edge0 = find_ice_edge(sic0, lsmask)
        with open(f"{icefile0}.npy", 'wb') as f:
            np.save(f, ice_edge0)

    else:
        print('loading edge0')
        with open(f"{icefile0}.npy", 'rb') as f:
            ice_edge0 = np.load(f)
    ...
    ax['a'].set_title(f"Input ice chart")
    ...
    icefile1 = 'icefile1'
    if not os.path.isfile(f"{path_figure}{icefile1}.npy"):
        print('creating edge1')
        ice_edge2 = find_ice_edge(sic2, lsmask)
        with open(f"{icefile1}.npy", 'wb') as f:
            np.save(f, ice_edge2)

    else:
        print('loading edge1')
        with open(f"{icefile1}.npy", 'rb') as f:
            ice_edge2 = np.load(f)
    ...
    ax['b'].set_title(f"Target ice chart")
    ...
    ax['c'].set_title(f"Deep learning forecast")
    ...
    # plt.tight_layout()
    print('saving fig')
    fig.savefig(f"{path_figure}split_predictions.png", dpi = 300)
