...
def main():
    ...
    plt.pcolormesh(np.ma.masked_array(a_plus, a_plus < 1), zorder=3, cmap='gray')
    plt.pcolormesh(np.ma.masked_array(a_minus, a_minus < 1), zorder=3, cmap=plt.colormaps['summer'])
    plt.pcolormesh(np.ma.masked_array(ocean, ocean < 1), zorder=3, cmap=plt.colormaps['Blues_r'])
    plt.pcolormesh(np.ma.masked_array(ice, ice < 1), zorder=3, cmap=plt.colormaps['cool'])
    # ax.pcolormesh(lon, lat, a_plus == 1, zorder=4, cmap='gray')
    plt.pcolormesh(np.ma.masked_array(lsmask, lsmask < 1), zorder=4, cmap='autumn', alpha = 0.2)
