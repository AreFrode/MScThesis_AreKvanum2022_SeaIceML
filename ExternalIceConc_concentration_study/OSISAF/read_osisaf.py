...
def runstuff():
    ...
    path_output = '/lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML/OSISAF_concentration_study/testingdata/'
    ...
    year, month, days, output = get_monthrange(osisaf_post2015, path_output, paths[2 - 1])
    ...
    for dd in range(1, days):
        ...
        try:
            # path_osisaf = glob.glob(f"{paths[2 - 1]}ice_conc_nh_ease2-250_cdr-v2p0_{yyyymmdd}1200.nc")[0]  # uncomment if pre 2016
            path_osisaf = glob.glob(f"{paths[2 - 1]}ice_conc_nh_ease2-250_icdr-v2p0_{yyyymmdd}1200.nc")[0]  # uncomment if post 2015
