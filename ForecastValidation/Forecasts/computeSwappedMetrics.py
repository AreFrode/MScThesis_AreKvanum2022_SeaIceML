...
def main():
    ...
    fname_slice = sys.argv[3]
    seed = sys.argv[4]

    PATH_FORECAST = f"/lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML/SimpleUNET/RunModel/outputs/Data/swapped/seed_{seed}/{model_name}/overwritten_{predictor}/"
    ...
    PATH_OUTPUTS = f"/lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML/ForecastValidation/lead_time_{config['lead_time']}/swapped/seed_{seed}/"
    ...
    for target, forecast in tzip(icecharts, forecasts):
        ...
        output_list.append([pd.to_datetime(date, format="%Y%m%d"), *NIIEE, *area_dist_target.tolist(), *area_dist_forecast.tolist()])
    ...
    output_df = pd.DataFrame(output_list, columns = ['date', *[f"NIIEE_{i}" for i in range(1, config['num_outputs'])], *[f"target_area{i}" for i in range(config['num_outputs'])], *[f"forecast_area{i}" for i in range(config['num_outputs'])]])

    output_df = output_df.set_index('date')
    output_df.to_csv(f"{PATH_OUTPUTS}{model_name}_overwritten_{predictor}.csv")
