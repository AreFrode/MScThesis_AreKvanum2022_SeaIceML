...
def main():
    ...
    figname = f"{PATH_FIGURES}model_intercomparisson_less_{weights}.pdf"
    ...
    categories = ['Very open drift ice', 'Open drift ice', 'Close drift ice', 'Very close drift ice']
    ...
    for i, lab, cat, cont in zip(range(1, 5), mosaic_labels, categories, contours):
        ...
        if lab in mosaic_labels[:2]:
            axs[lab].set_xticklabels([])
    
        if not lab in mosaic_labels[:2]:
