...
for i in {1..3}; do singularity exec -B /lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML:/mnt --nv $HOME/TFcontainer/tensorflow_latest-gpu.sif sh -c "$COMMAND $i > /mnt/SimpleUNET/RunModel/logs/$(date -d "today" +"%d%m%H%M").log 2> /mnt/SimpleUNET/RunModel/errors/$(date -d "today" +"%d%m%H%M").log"; done