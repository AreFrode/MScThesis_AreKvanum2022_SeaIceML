...
from dataset import HDF5Generator, NoisyMultiOutputHDF5Generator
from datetime import datetime, timedelta
from helper_functions import read_config_from_csv
from predict_validation import numpy_where_wrapper

def predict_test_multi(test_generator, model, PATH_OUTPUTS, weights, lead_time, name):
    ...
    for i in range(samples):
        ...
        hdf_path = f"{PATH_OUTPUTS}Data/noisy/{weights}/overwritten_{name}/{yyyymmdd[:4]}/{yyyymmdd[4:6]}/"
...
def main():    
    ...
    for i, name in zip(range(len(overwritten)), overwritten):
        test_generator = NoisyMultiOutputHDF5Generator(data_2022, 
        ...
        predict_test_multi(test_generator, model, PATH_OUTPUTS, weights, config['lead_time'], name)
