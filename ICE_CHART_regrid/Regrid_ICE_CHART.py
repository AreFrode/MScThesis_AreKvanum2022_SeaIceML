...
def runstuff():
	...
	path_output = '/lustre/storeB/users/arefk/MScThesis_AreKvanum2022_SeaIceML/ICE_CHART_regrid/testingdata/'
	...
	# X_AROMEgrid, Y_AROMEgrid = np.meshgrid(x_AROMEgrid, y_AROMEgrid)
	...
	for dd in range(1, nb_days_task + 1):
		...
		path_arome_day = glob.glob(f"{path_arome}{year_task}/{month_task}/{dd:02d}/arome_arctic_full_2_5km_{yyyymmdd}T00Z.nc")

		try:
			dataset = path_day[0]
			dataset_arome = path_arome_day[0]
		...
		# nc_arome = Dataset(dataset_arome, 'r')

		# x_arome = nc_arome.variables['x']
		# y_arome = nc_arome.variables['y']
		# xx_arome, yy_arome = np.meshgrid(x_arome, y_arome)
		# xx_arome_flat = xx_arome.flatten()
		# yy_arome_flat = yy_arome.flatten()

		# t2m_flat = np.mean(nc_arome.variables['air_temperature_2m'][0:24,...][0], axis=0).flatten()

		# t2m_REgrid = griddata((xx_arome_flat, yy_arome_flat), t2m_flat, (x_AROMEgrid[None, :], y_AROMEgrid[:, None]), method='nearest')
		...
		# t2m = output_netcdf.createVariable('t2m', 'd', ('y', 'x'))
		# t2m.units = 'K'
