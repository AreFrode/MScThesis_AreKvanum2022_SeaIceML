...
def get_common_boundaries(path_current: str, path_target: str, crs_current: pyproj_crs, crs_target: pyproj_crs) -> List[List[List]]:
    ...
    x_current = nc_current.variables['xc'][:]
    y_current = nc_current.variables['yc'][:]
    x_target = nc_target.variables['x'][:]
    y_target = nc_target.variables['y'][:]

    print(f"{x_current.shape=}")
    print(f"{y_current.shape=}")
    print(f"{x_target.shape=}")
    print(f"{y_target.shape=}")
...
def runstuff():
    ...
    coords = get_common_boundaries(path_icechart, path_arome, crs_ICECHART, crs_AROME)
